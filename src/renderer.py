...
def render_file(in_file_path: pathlib.Path, path_working: pathlib.Path, group: conf_reader.Group, use_symlinks: bool):
    print(f"Running processor {group.process} on {in_file_path} into {path_working}")
    processing.process_dict[group.process](in_file_path, path_working)
...
def render_group(group: conf_reader.Group, dir_working, use_symlinks=True):
    ...
    if in_path_obj.is_dir():
        for (dir_path, dirs, files) in in_path_obj.walk(follow_symlinks=True):
            for fn in files:
                ...
                render_file(dir_path / fn, path_working, group, use_symlinks)
    elif in_path_obj.is_file():
        path_working = dir_working_group / f"{in_path_obj.name}_{random.randint(0, 9999) :0>4}"
        render_file(in_path_obj, path_working, group, use_symlinks)
    else:
